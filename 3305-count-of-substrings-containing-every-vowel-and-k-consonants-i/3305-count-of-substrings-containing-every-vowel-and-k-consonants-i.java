class Solution {
    ...
    public int substrings(String word,int k)// atmost k consonants + atleast all vowels covered
    {
       int a=0,e=0,i=0,o=0,u=0;
       int La=-1,Le=-1,Li=-1,Lo=-1,Lu=-1; // last seen indices
       int count_k=0;
       int l=0,r=0;
       int count_substrings=0;
       int minLastScene=0;// Minimum of last index of any vowel in the window
       while(r<word.length())
       {
         ...
         if(a>=1 && e>=1 && i>=1 && o>=1 && u>=1){
           int minLastSeen = Math.min(La, Math.min(Le, Math.min(Li, Math.min(Lo, Lu))));
